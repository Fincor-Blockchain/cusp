...
import (
	...
	clientkeys "github.com/fincor/cusp-sdk/client/keys"
	"github.com/fincor/cusp-sdk/codec"
	"github.com/fincor/cusp-sdk/crypto/keys"
	...
	"github.com/fincor/cusp-sdk/x/distribution"
...
// GDfincor returns the results of cuspd pou [query]
func (f *Fixtures) GDfincor(query string) string {
	cmd := fmt.Sprintf("%s pou %s --home=%s", f.CuspdBinary, query, f.CuspdHome)
...
func WriteToNewTempFile(t *testing.T, s string) *os.File {
	fp, err := ioutil.TempFile(os.TempDir(), "fincor_cli_test_")
	require.Nil(t, err)
